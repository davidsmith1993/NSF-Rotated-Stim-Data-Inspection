function [error,corr] = calc_error(MyData,or_scale,diameter_scale,inferDimensionCol,diameter_responseCol,diameterCol,orient_responseCol,orientCol)
%function [error,corr] = calc_error(MyData,o_gain,inferDimensionCol,diameter_responseCol,diameterCol,orient_responseCol,orientCol)
%
...
%correlations
temp = corrcoef(MyData(ind,diameter_responseCol),MyData(ind,orientCol)*or_scale);
corr(1) = temp(1,2);
...
%correlations
temp = corrcoef(MyData(ind,diameterCol)*diameter_scale,MyData(ind,orient_responseCol));
corr(2) = temp(1,2);
